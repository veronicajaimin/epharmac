...
class HomeController extends Controller
{
   ...
   { 
        ...
        if(Auth::id())
        {
            ...
            return view('home.show_category', compact('product', 'count'));
         }
 
        else
        {
            $product=Product::paginate(10);
            return view('home.show_category', compact('product'));
        }
    }
   ...
   {
        $order=order::find($id); //find specific $id in the order table to update the data

        $order->delivery_status='Order has been cancelled';
   ...
   public function product_search(Request $request)
    {
        ...
        {   
        $cari=$request->search;
        $product=product::where('title', 'LIKE', "%$cari%")->orWhere('category', 'LIKE', "$cari")->paginate(10);
